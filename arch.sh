...
setup_defaults() {
    ...
    /usr/bin/printf 'ntf "#     %%s\\n" "$i"\n    done\n}\nstatus_services() {\n    if [ -e "/usr/bin' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf '/pacman" ]; then\n        if [ -f "/var/run/updates.list" ]; then\n            u' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'pdates="$(wc -l < "/var/run/updates.list") Pending"\n        else\n            i' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'f [ "$UID" = "0" ]; then\n                systemctl start checkupdates.service\n' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf '                updates="Checking for updates.."\n            else\n            ' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf '    updates="Updates check pending.."\n            fi\n        fi\n        print' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'f "# Updates:\\t%%s\\n" "${updates}"\n    fi\n    printf "# Network:\\t%%s Estab' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'lished, " "$(netstat -panut 2> /dev/null | grep -c "ESTABLISHED")"\n    printf "' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf '%%s Listening\\n" "$(netstat -panut 2> /dev/null | grep -c "LISTEN")"\n    print' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'f "# Services:\\t%%s Running, " "$(systemctl --state=active --no-legend --no-pag' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'er | grep ".service" | grep -c "running")"\n    printf "%%s Failed, " "$(systemc' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'tl --state=failed --no-legend --no-pager | grep -c ".service")"\n    printf "%%s' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf ' Timers\\n" "$(systemctl --all --no-legend --no-pager | grep -c ".timer")"\n}\n\n' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'if [[ $- != *i* ]] || [ -n "$SSH_CLIENT" ]; then\n    printf "##################' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf '############################################\\n"\n    status_storage\n    status' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf '_network\n    printf "# Uptime:\\t%%s\\n" "$(uptime --pretty | sed '\''s/up //g'\''' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf ')"\n    printf "# Kernel:\\t%%s\\n" "$(uname -r)"\n    status_repo\n    status_s' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf 'ervices\n    printf "###########################################################' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
    /usr/bin/printf '###\\n"\nfi\n' >> "${SETUP_DIRECTORY}/etc/profile.d/z_system_status.sh"
