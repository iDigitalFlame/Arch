...
setup_config() {
    ...
    /usr/bin/printf 'hmod 0500 "${SYSCONFIG}/etc/iptables"\nchmod 0444 "${SYSCONFIG}/etc/resolv.conf"' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    /usr/bin/printf '\nchmod 0500 "${SYSCONFIG}/etc/pacman.d/hooks"\nchmod 0400 "${SYSCONFIG}/etc/mki' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    /usr/bin/printf 'nitcpio.conf"\nchmod 0440 "${SYSCONFIG}/etc/ssh/sshd_config"\nchmod 0500 "${SYSC' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    /usr/bin/printf 'ONFIG}/etc/security/limits.d"\nchmod 0400 "${SYSCONFIG}/etc/sysctl.d/kernel.conf' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    /usr/bin/printf '"\nchmod 0400 "${SYSCONFIG}/etc/systemd/coredump.conf"\nchmod 0400 "${SYSCONFIG}' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    /usr/bin/printf '/etc/iptables/iptables.rules"\nchmod 0400 "${SYSCONFIG}/etc/iptables/ip6tables.r' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    ...
    /usr/bin/printf 'oot -R "/etc/systemd"\nchown root:root -R "/etc/iptables"\nchown root:root -R "/' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    /usr/bin/printf 'etc/pacman.d"\nchown root:root -R "/etc/security"\nchown root:root -R "/etc/sysc' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    /usr/bin/printf 'tl.d"\nchown root:root -R "/etc/profile.d"\nchown root:root -R "/etc/syscheck.d"' >> "${ROOT}${SYSCONFIG_DIR}/bin/syslink"
    ...
    /usr/bin/printf 'xit 1\nfi\nif ! [ -d "$SYSCONFIG" ]; then\n    printf "System Configuration dire' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspull"
    /usr/bin/printf 'ctory \\"%%s\\" does not exist!\\n" "$SYSCONFIG"\n    exit 1\nfi\n\nSYSCONFIG=${' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspull"
    /usr/bin/printf 'SYSCONFIG%%/}\nif ! [ -d "${SYSCONFIG}/.git" ]; then\n    printf "System Configu' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspull"
    /usr/bin/printf 'ration directory \\"%%s\\" is not a Git repository!\\n" "$SYSCONFIG"\n    exit 1' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspull"
    /usr/bin/printf '\nfi\n\nif ! [ "$UID" = "0" ]; then\n    echo "Only root can do this!"\n    exit' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspull"
    ...
    /usr/bin/printf 'xit 1\nfi\nif ! [ -d "$SYSCONFIG" ]; then\n    printf "System Configuration dire' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspush"
    /usr/bin/printf 'ctory \\"%%s\\" does not exist!\\n" "$SYSCONFIG"\n    exit 1\nfi\n\nSYSCONFIG=${' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspush"
    /usr/bin/printf 'SYSCONFIG%%/}\nif ! [ -d "${SYSCONFIG}/.git" ]; then\n    printf "System Configu' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspush"
    /usr/bin/printf 'ration directory \\"%%s\\" is not a Git repository!\\n" "$SYSCONFIG"\n    exit 1' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspush"
    /usr/bin/printf '\nfi\n\nif ! [ "$UID" = "0" ]; then\n    echo "Only root can do this!"\n    exit' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspush"
    /usr/bin/printf ' 1\nfi\n\nsh -c "cd ${SYSCONFIG}; git add -A .; git commit -m '\''syspush'\''; g' >> "${ROOT}${SYSCONFIG_DIR}/bin/syspush"
    ...
    /usr/bin/printf 'rectory \\"%%s\\" does not exist!\\n" "$SYSCONFIG"\n    exit 1\nfi\n\nif [ "$1" ' >> "${ROOT}${SYSCONFIG_DIR}/bin/sysupdate"
    /usr/bin/printf '== "--auto" ]; then\n    if [ "$2" == "--download" ]; then\n        pacman -Syuw' >> "${ROOT}${SYSCONFIG_DIR}/bin/sysupdate"
    /usr/bin/printf ' --noconfirm\n        exit 0\n    fi\n    pacman -Syu --noconfirm\nelse\n    pac' >> "${ROOT}${SYSCONFIG_DIR}/bin/sysupdate"
    ...
    /usr/bin/printf 'xit 1\nfi\nif ! [ -d "$SYSCONFIG" ]; then\n    printf "System Configuration dire' >> "${ROOT}${SYSCONFIG_DIR}/etc/syscheck.d/git.sh"
    /usr/bin/printf 'ctory \\"%%s\\" does not exist!\\n" "$SYSCONFIG"\n    exit 1\nfi\n\nif [ -d "${S' >> "${ROOT}${SYSCONFIG_DIR}/etc/syscheck.d/git.sh"
    /usr/bin/printf 'YSCONFIG}/.git" ]; then\n    chmod 0750 "$SYSCONFIG"/.git/hooks/*\n    bash -c "' >> "${ROOT}${SYSCONFIG_DIR}/etc/syscheck.d/git.sh"
