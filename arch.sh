...
setup_config() {
    ...
    if [ "$_rw_type" = "ro" ]; then
        ...
        # Fix for systemd-resolved loading before the 'var' drive is mounted.
        mkdir "${SETUP_DIRECTORY}/etc/systemd/system/systemd-resolved.service.d"
...
setup_defaults() {
    ...
    /usr/bin/printf 'system configuration directory \\"%%s\\" is not a repository!\\n" "$SYSCONFIG"\n' >> "${SETUP_DIRECTORY}/bin/syspull"
    ...
    /usr/bin/printf '/locale/compose.dir !usr/share/X11/locale/iso8859-1/*\nNoExtract           = !*l' >> "${SETUP_DIRECTORY}/etc/pacman.conf"
    /usr/bin/printf 'ocale*/en*/* !usr/share/*locale*/locale.*\nNoExtract           = !usr/share/*loc' >> "${SETUP_DIRECTORY}/etc/pacman.conf"
    /usr/bin/printf 'ales/en_?? !usr/share/*locales/i18n* !usr/share/*locales/iso*\nNoExtract        ' >> "${SETUP_DIRECTORY}/etc/pacman.conf"
    ...
    /usr/bin/printf 'C !usr/share/*locales/POSIX\nNoExtract           = usr/share/vim/vim*/lang/*\nNo' >> "${SETUP_DIRECTORY}/etc/pacman.conf"
    /usr/bin/printf 'Extract           = usr/share/gtk-doc/html/*\nNoExtract           = usr/share/do' >> "${SETUP_DIRECTORY}/etc/pacman.conf"
    /usr/bin/printf 'c/HTML/*\nNoExtract           = usr/share/help/* !usr/share/help/C/*\n\nVerboseP' >> "${SETUP_DIRECTORY}/etc/pacman.conf"
    /usr/bin/printf 'kgLists\n\n[core]\nInclude             = /etc/pacman.d/mirrorlist\n[extra]\nIncl' >> "${SETUP_DIRECTORY}/etc/pacman.conf"
    ...
    /usr/bin/printf '      curve25519-sha256@libssh.org,diffie-hellman-group-exchange-sha256\nLoginGr' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    /usr/bin/printf 'aceTime                  1m\nPermitRootLogin                 yes # prohibit-pass' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    /usr/bin/printf 'word\nAllowTcpForwarding              no\nAuthorizedKeysFile              .ssh/a' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    /usr/bin/printf 'uthorized_keys\nClientAliveCountMax             0\nClientAliveInterval          ' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    /usr/bin/printf '   600\nIgnoreUserKnownHosts            no\nPermitEmptyPasswords            no\n' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    /usr/bin/printf 'AllowAgentForwarding            no\nPubkeyAuthentication            yes\nAuthent' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    /usr/bin/printf 'icationMethods           publickey password # publickey\nPasswordAuthentication ' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    /usr/bin/printf '         yes # no\nKerberosAuthentication          no\nHostbasedAuthentication  ' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
    ...
    /usr/bin/printf 'fs.file-max                      = 372925\nkernel.sysrq                     = 16' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf '\nvm.swappiness                    = 10\nfs.suid_dumpable                 = 0\nf' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf 's.protected_fifos               = 1\nfs.protected_regular             = 1\nkerne' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf 'l.kptr_restrict             = 2\nkernel.dmesg_restrict            = 1\nfs.protec' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf 'ted_symlinks            = 1\nfs.protected_hardlinks           = 1\nkernel.yama.p' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf 'trace_scope         = 3\nkernel.kexec_load_disabled       = 1\nfs.inotify.max_us' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf 'er_watches      = 16777215\nfs.inotify.max_queued_events     = 32767\nvm.dirty_w' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf 'riteback_centisecs     = 1500\nfs.inotify.max_user_instances    = 4096\nkernel.u' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    /usr/bin/printf 'nprivileged_bpf_disabled = 1\n' >> "${SETUP_DIRECTORY}/etc/sysctl.d/kernel.conf"
    ...
    /usr/bin/printf 'XDG_CACHE_HOME="/var/cache/pacman"\nKeyringMode             = private\nProtectHo' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
    /usr/bin/printf 'me             = false\nPrivatePIDs             = true\nProtectClock            ' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
    /usr/bin/printf '= true\nPrivateUsers            = true\nProtectSystem           = strict\nReadWr' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
    /usr/bin/printf 'itePaths          = /var/cache/pacman\nPrivateDevices          = true\nNoNewPriv' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
    /usr/bin/printf 'ileges         = true\nLockPersonality         = true\nRestrictRealtime        =' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
    /usr/bin/printf ' true\nRestrictSUIDSGID        = true\nProtectKernelLogs       = true\nRestrictN' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
    /usr/bin/printf 'amespaces      = true\nProtectControlGroups    = true\nProtectKernelModules    =' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
    /usr/bin/printf ' true\nProtectKernelTunables   = true\nRestrictAddressFamilies = AF_INET AF_INET' >> "${SETUP_DIRECTORY}/etc/systemd/system/reflector.service"
