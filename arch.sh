...
setup_defaults() {
    ...
    /usr/bin/printf '           = description\nKExecWatchdogSec                = off\nDefaultRestartS' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'ec               = 100ms\nDefaultLimitNOFILE              = 1024:524288\nRuntime' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'WatchdogSec              = 0\nShutdownWatchdogSec             = 0\nDefaultIOAcco' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'unting             = no\nDefaultIPAccounting             = no\nDefaultStandardEr' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'ror            = inherit\nDefaultCPUAccounting            = no\nDefaultTimeoutSt' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'opSec           = 30s\nDefaultStandardOutput           = journal\nCtrlAltDelBurs' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'tAction           = reboot-force\nDefaultTimeoutStartSec          = 30s\nDefault' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'StartLimitBurst          = 5\nDefaultTasksAccounting          = yes\nDefaultTime' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'rAccuracySec         = 1min\nDefaultMemoryAccounting         = yes\nDefaultBlock' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
    /usr/bin/printf 'IOAccounting        = no\nDefaultStartLimitIntervalSec    = 10s\n' >> "${SETUP_DIRECTORY}/etc/systemd/system.conf"
